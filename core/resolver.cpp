...
vector<string> GetRpaths2(const LIEF::ELF::Binary *binary, const fs::path &path)
{
...
static result<vector<DynamicLibrary>> ResolveDependenciesImpl(const fs::path &path, bool recurse,
                                                              unordered_flat_map<string, bool> &visited)
{
    ...
    auto rpaths = GetRpaths2(elf.get(), path);
    ...
    for (const auto &lib : elf->imported_libraries())
    {
        ...
        dependencies.push_back(DynamicLibrary{
            .Name = lib, .Path = *libPath, .Versions = (it != versions.end()) ? it->second : vector<string>{}});
                                                              ...
                                                              unordered_flat_map<string, bool> &visited)
{
    ...
    for (const auto &lib : pe->imported_libraries())
    {
        ...
        dependencies.push_back(
            DynamicLibrary{.Name = lib, .Path = libPath.value_or(fs::path(lib)), .Versions = vector<string>{}});
...
result<AnalysisResult> AnalysisResult::AnalyzeExecutable(const fs::path &executable, bool recurse)
{
    ...
    return AnalysisResult{
        .Dependencies = std::move(dependencies), .MinimalGlibc = minimalGlibc, .MinimalGlibcxx = minimalGlibcpp};
}
} // namespace dyneeded